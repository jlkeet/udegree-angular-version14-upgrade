...
export class CourseDetails {
  ...
  @Input() public course: ICourse = null as any;
  @Input() public showSemesterFullMessage: boolean = false;
  @Input() public showAddCourse: boolean = false;
  @Input() public messages: string[] = [];
  @Input() public messageRequirements: IRequirement[] = [];
  ...
  public courseGrade: any;

  public departments: any;
  public departmentOptions: any;
  public faculties: any;
  public facultyOptions: any;
  public conjoints: any;
  public conjointOptions: any;
  public pathways: any;
  public pathwayOptions: any;
  public modules: any;
  public moduleOptions: any;

  public customTitle: any;
  public customPoints: any;
  public customCode: any;
  public customFaculty: any;
  public customDepartment: any;
   ...
   */
  @HostListener("document:click", ["$event"]) public clickedOutside($event: any) {
    if (this.opened) {
      this.closeDetails();
    } else {
      this.opened = true;
...
public async getAllMaps() {
  ...
  this.departmentOptions = this.departments.map((department: any) => {
    return { label: department.name, value: department.name };
  });

  this.facultyOptions = this.faculties.map((faculty: any) => {
    return { label: faculty.name, value: faculty.name };
  });

  this.conjointOptions = this.conjoints.map((conjoint: any) => {
    return { label: conjoint.name, value: conjoint.name };
  });

  this.pathwayOptions = this.pathways.map((pathway: any) => {
    return { label: pathway.name, value: pathway.name };
  });

  this.moduleOptions = this.modules.map((module: any) => {
    return { label: module.name, value: module.name };
  });
