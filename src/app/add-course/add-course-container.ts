...
export class AddCourseContainer {
  ...
  public custom: any;
  private selectedTempCardId: number | null = null;
  ...
  constructor(
   ...
   //   (defaultFaculties[0] ? defaultFaculties.map((faculty) => faculty.name) : []);
    ...
    this.selectedTempCardId = this.parseTempCardId(
      this.route.snapshot.queryParams["tempCardId"]
    );
    this.semesters = this.storeHelper.current('semesters');
    const routeTempCard = this.getRouteTempCard();
    if (routeTempCard) {
      this.coursesService.tempCard = routeTempCard;
    }
  ...
  public async toggleCourse(courseToToggle: ICourse) {
    ...
    if (alreadyPlanned) {
      ...
      await this.consumeSelectedTempCard();
    }
  }

  public async addCourse(event: any) {
  ...
  private parseTempCardId(raw: any): number | null {
  ...
  private getRouteTempCard(): any | null {
    if (this.selectedTempCardId === null) {
      return null;
    }
    ...
    const semester = semesters.find(
      (entry: any) =>
        Number(entry.year) === Number(this.year) &&
        Number(entry.period) === Number(this.period)
    );
    if (!semester) {
      return null;
    }
    ...
    return (
      tempCards.find(
        (card: any) => Number(card.generatedId) === Number(this.selectedTempCardId)
      ) || null
    );
  }

  private async consumeSelectedTempCard(): Promise<void> {
    const selectedTempCard = this.getRouteTempCard() || this.coursesService.tempCard;
    if (!selectedTempCard) {
      return;
    }

    try {
      await this.coursesService.deleteTempCard(selectedTempCard);
    } catch (error) {
      console.error("Failed to consume temp card after adding course:", error);
    } finally {
      if (
        this.coursesService.tempCard &&
        this.coursesService.tempCard.generatedId === selectedTempCard.generatedId
      ) {
        this.coursesService.tempCard = null;
      }
      this.selectedTempCardId = null;
    }
  }
