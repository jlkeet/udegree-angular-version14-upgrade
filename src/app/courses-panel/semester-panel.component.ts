...
export class SemesterPanel {
  ...
  public async selectPath(path: any) {
    ...
    const stageFilters = this.normaliseTempCardStages(path);
    const isGeneral = this.pathHasGeneralFlag(path);
    ...
    if (path.papers) {
      ...
      const { departments, faculties, required } = path;
      const numCards = Math.ceil(required / 15);
  
      // Generate individual cards for departments or faculties
      for (let i = 0; i < numCards; i++) {
        const card: TempCard = {
          department: Array.isArray(departments) && departments.length > 0 ? departments[0] : null,
          departments: Array.isArray(departments) && departments.length > 0 ? departments : null,
          faculty: Array.isArray(faculties) && faculties.length > 0 ? faculties[0] : null,
          faculties: Array.isArray(faculties) && faculties.length > 0 ? faculties : null,
          level: stageFilters.length === 1 ? stageFilters[0] : null,
          stages: stageFilters.length > 1 ? stageFilters : null,
          general: isGeneral,
          points: 15,
          value: 15,
          generatedId: Math.floor(Math.random() * 100000),
    ...
    } else if (!path.departments && !path.faculties && !path.papers) {
      const { required } = path;
      const numCards = Math.ceil(required / 15);
  ...
  private async saveCardToDatabase(card: any) {
    try {
      const semesterId = await this.sampleService.getSemesterId(
        this.semester.year,
        this.semester.period,
        true
      );
  ...
  public getTempCardDepartments(tempCard: TempCard): string[] | string | null {
    if (Array.isArray(tempCard.departments) && tempCard.departments.length > 0) {
      return tempCard.departments;
    }

    return tempCard.department;
  }

  public getTempCardFaculties(tempCard: TempCard): string[] | string | null {
    if (Array.isArray(tempCard.faculties) && tempCard.faculties.length > 0) {
      return tempCard.faculties;
    }

    return tempCard.faculty;
  }

  public describeTempCard(tempCard: TempCard): string {
    if (tempCard.paper) {
      return tempCard.paper;
    }

    if (this.isGeneralTempCard(tempCard)) {
      const stages = this.getTempCardQueryStages(tempCard) || [];
      const hasLevelConstraint =
        (typeof tempCard.level === "number" && tempCard.level > 0) ||
        (Array.isArray(tempCard.stages) && tempCard.stages.length > 0);
      if (hasLevelConstraint) {
        return `${this.formatStageLabel(stages)} General Education Course`;
      }

      return "General Education Course";
    }

    const scope = this.getTempCardScope(tempCard);
    const stages = this.getTempCardQueryStages(tempCard) || [];
    const hasLevelConstraint =
      (typeof tempCard.level === "number" && tempCard.level > 0) ||
      (Array.isArray(tempCard.stages) && tempCard.stages.length > 0);
    ...
    return `${this.formatStageLabel(stages)} ${scope}`;
  }
  ...
  public getTempCardGeneral(tempCard: TempCard): boolean | null {
    return this.isGeneralTempCard(tempCard) ? true : null;
  }
  ...
  private formatStageLabel(stages: number[]): string {
    ...
    if (sorted.length === 1) {
      return `${sorted[0]}00-level`;
    }

    return `${sorted.map((stage: number) => `${stage}00`).join("/")} level`;
  }
  ...
  private pathHasGeneralFlag(path: any): boolean {
    if (path?.general === true) {
      return true;
    }
    ...
    if (typeof flags === "object") {
      return flags.General === true || flags.general === true;
    }

    return false;
  }

  private isGeneralTempCard(tempCard: TempCard): boolean {
    if (tempCard.general) {
      return true;
    }
    ...
    // Backward compatibility for older generated cards that encoded Gen Ed as a large faculty list.
    return faculties.length >= 4;
  }

  private availableStages(): number[] {
    const allCourses = Array.isArray(this.courseService.allCourses)
